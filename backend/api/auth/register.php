...
try {
    ...
    $role = isset($data['role']) && in_array($data['role'], ['admin', 'chauffeur', 'responsable', 'tuteur']) 
        ? $data['role'] 
        : 'tuteur';
    ...
    $stmt = $pdo->prepare('SELECT id, nom, prenom, email, telephone, role, statut, date_creation FROM utilisateurs WHERE id = ?');
    $stmt->execute([$userId]);
